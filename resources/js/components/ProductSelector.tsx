...
import { ArrowRight, Check, ChevronLeft, Filter } from 'lucide-react';
...
export default function ProductSelector() {
    ...
    const handleBack = () => {
        if (currentStep > 0) {
            const previousPaso = pasosWizard[currentStep - 1];
    ...
    const isComplete = pasosWizard.every(paso => selections[paso.identificador]);
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center mb-12">
                    <div className="inline-flex items-center gap-2 bg-[#EA0A2A]/10 rounded-full px-4 py-2 mb-4">
                        <Filter size={16} className="text-[#EA0A2A]" />
                ...
                <div className="max-w-3xl mx-auto mb-12">
                    ...
                    <div className="flex justify-between text-xs text-gray-500">
                        {pasosWizard.map((paso: any) => (
                            <span key={paso.id}>{paso.identificador.charAt(0).toUpperCase() + paso.identificador.slice(1)}</span>
                        ))}
                    </div>
                </div>
                        ...
                        <div className="bg-white rounded-2xl shadow-xl p-8 md:p-12">
                            ...
                            <div className="flex justify-between mt-8 pt-6 border-t border-gray-100">
                                <button
                                    ...
                                    className="flex items-center gap-2 px-6 py-3 rounded-lg font-semibold text-gray-600 hover:text-gray-900 disabled:opacity-30 disabled:cursor-not-allowed transition-all"
                                >
                                    <ChevronLeft size={20} />
                                    Anterior
                                </button>
                                <button
                                    onClick={handleReset}
                                    className="px-6 py-3 rounded-lg font-semibold text-gray-500 hover:text-gray-700 transition-all"
                                >
                                    Reiniciar
                                </button>
                            </div>
                        ...
                        <div className="bg-gradient-to-br from-[#EA0A2A] to-[#c90825] rounded-2xl shadow-2xl p-8 md:p-12 text-white">
                                ...
                                <div className="space-y-3">
                                    {pasosWizard.map((paso: any) => (
                                        <div key={paso.id} className="flex items-center gap-3">
                                            <span className="text-white/70 text-sm capitalize">{paso.identificador}:</span>
                                            <span className="font-semibold">{selections[paso.identificador]?.nombre}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            {selections.producto && selections.categoria && (
                                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                                    <Link
                                        href={`/products/${selections.producto.slug}/${selections.categoria.slug}`}
                                        className="bg-white text-[#EA0A2A] px-8 py-4 rounded-lg font-bold hover:bg-gray-100 transition-all flex items-center justify-center gap-2"
                                    >
                                    ...
                                    <button
                                        onClick={handleReset}
                                        className="bg-white/10 hover:bg-white/20 text-white px-8 py-4 rounded-lg font-semibold transition-all border border-white/20"
                                    >
